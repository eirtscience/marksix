...
just extend with do_something  method to handle your commands"""
    ...
    def help(self, cmd=None):
        def std_help():
            ...
            cl = [name[3:] for name in dir(
                self) if name.startswith('do_') and len(name) > 3]
    ...
    def on_line_entered(self, line):
        if self._cmd:
            try:
                ...
                self.output('Error: {}'.format(e), 'error')
                return
...
class TestCmd(Command):
    ...
    def show_draw(self, data=None):
        ...
        if data:
            ...
            draw_data = self.emitter.getData()
...
Win            : {}
    ...
    def check_result(self):
        try:
            ...
            while True:
                if self.emitter:
                    data = self.emitter.emmit_draw()
                    if data.get("draw_status") == "completed":
                        break
                time.sleep(5)

            self.body.append(self.show_draw(data))
        except AttributeError:
            pass
